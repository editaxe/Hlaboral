...
$sql_historia_clinica = "SELECT Count(cod_estado_facturacion) AS conteo_estado_facturacion, cod_estado_facturacion FROM historia_clinica 
...
FROM historia_clinica GROUP BY $tipo_fecha, cod_estado_facturacion, nombre_empresa 
HAVING ((cod_estado_facturacion=1)) ORDER BY fecha_ymd ASC";
$resultado_factura = mysqli_query($conectar, $sql_factura);

$contador = 0;

while ($info_factura = mysqli_fetch_assoc($resultado_factura) ) { 
$contador++;

$fecha_mes                     = $info_factura['fecha_mes'];
$fecha_ymd                     = $info_factura['fecha_ymd'];
$cod_estado_facturacion        = $info_factura['cod_estado_facturacion'];
$nombre_empresa_ciclo          = $info_factura['nombre_empresa'];
...
$sql_motivo_conteo = "SELECT Count(motivo) AS conteo_motivo, $tipo_fecha, cod_estado_facturacion, nombre_empresa, motivo FROM historia_clinica
...
$sql_motivo_costo = "SELECT cod_motivo_consulta, motivo, costo_motivo_consulta FROM motivo_consulta WHERE motivo = '$motivo'";
