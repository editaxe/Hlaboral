...
<?php if (isset($fecha_mes)) { echo "<option value='-1' >...</option>";
} else { echo  "<option value='-1' selected >...</option>"; }
$sql_consulta = ("SELECT fecha_mes FROM historia_clinica GROUP BY fecha_mes ORDER BY fecha_ymd DESC");
$resultado = mysqli_query($conectar, $sql_consulta);
while ($contenedor = mysqli_fetch_assoc($resultado)) {
...
historia_clinica.cod_administrador, historia_clinica.cod_cliente, historia_clinica.fecha_mes, historia_clinica.cod_factura, historia_clinica.costo_motivo_consulta,
...
WHERE ((historia_clinica.nombre_empresa)='$nombre_empresa') AND ((historia_clinica.cod_estado_facturacion)=1) AND ((historia_clinica.fecha_mes)='$fecha_mes') ORDER BY historia_clinica.fecha_time DESC";
...
FROM empresa RIGHT JOIN (cliente RIGHT JOIN (administrador RIGHT JOIN historia_clinica ON administrador.cod_administrador = historia_clinica.cod_administrador) 
ON cliente.cod_cliente = historia_clinica.cod_cliente) ON empresa.nombre_empresa = historia_clinica.nombre_empresa
WHERE ((historia_clinica.nombre_empresa)='$nombre_empresa') AND ((historia_clinica.fecha_mes)='$fecha_mes')";
...
FROM empresa RIGHT JOIN (cliente RIGHT JOIN (administrador RIGHT JOIN historia_clinica ON administrador.cod_administrador = historia_clinica.cod_administrador) 
ON cliente.cod_cliente = historia_clinica.cod_cliente) ON empresa.nombre_empresa = historia_clinica.nombre_empresa
WHERE ((historia_clinica.nombre_empresa)='$nombre_empresa') AND ((historia_clinica.cod_estado_facturacion)=1) AND ((historia_clinica.fecha_mes)='$fecha_mes')";
...
FROM empresa RIGHT JOIN (cliente RIGHT JOIN (administrador RIGHT JOIN historia_clinica ON administrador.cod_administrador = historia_clinica.cod_administrador) 
ON cliente.cod_cliente = historia_clinica.cod_cliente) ON empresa.nombre_empresa = historia_clinica.nombre_empresa
WHERE ((historia_clinica.nombre_empresa)='$nombre_empresa') AND ((historia_clinica.cod_estado_facturacion)=0) AND ((historia_clinica.fecha_mes)='$fecha_mes')";
...
/* --------------------------------------------------------------------------------------------------------------------------------- */
/* --------------------------------------------------------------------------------------------------------------------------------- */
$sql_conteo_atendido_hombre = "SELECT Count(cliente.nombre_sexo) AS conteo_hombre, historia_clinica.nombre_empresa, historia_clinica.cod_estado_facturacion, cliente.nombre_sexo, historia_clinica.fecha_mes
...
GROUP BY historia_clinica.nombre_empresa, historia_clinica.cod_estado_facturacion, cliente.nombre_sexo, historia_clinica.fecha_mes
HAVING (((historia_clinica.nombre_empresa)='$nombre_empresa') AND ((historia_clinica.cod_estado_facturacion)=1) AND ((cliente.nombre_sexo)='M') AND ((historia_clinica.fecha_mes)='$fecha_mes'))";
...
/* --------------------------------------------------------------------------------------------------------------------------------- */
/* --------------------------------------------------------------------------------------------------------------------------------- */
$sql_conteo_atendido_mujer = "SELECT Count(cliente.nombre_sexo) AS conteo_mujer, historia_clinica.nombre_empresa, historia_clinica.cod_estado_facturacion, cliente.nombre_sexo, historia_clinica.fecha_mes
FROM empresa RIGHT JOIN (cliente RIGHT JOIN (administrador RIGHT JOIN historia_clinica ON administrador.cod_administrador = historia_clinica.cod_administrador) ON cliente.cod_cliente = historia_clinica.cod_cliente) ON empresa.nombre_empresa = historia_clinica.nombre_empresa
GROUP BY historia_clinica.nombre_empresa, historia_clinica.cod_estado_facturacion, cliente.nombre_sexo, historia_clinica.fecha_mes
HAVING (((historia_clinica.nombre_empresa)='$nombre_empresa') AND ((historia_clinica.cod_estado_facturacion)=1) AND ((cliente.nombre_sexo)='F') AND ((historia_clinica.fecha_mes)='$fecha_mes'))";
