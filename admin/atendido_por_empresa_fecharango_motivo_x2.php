...
<?php if (isset($motivo)) { echo "<option value='-1' >...</option>";
} else { echo  "<option value='-1' selected >...</option>"; }
$sql_consulta = ("SELECT motivo FROM motivo_consulta GROUP BY motivo ORDER BY motivo DESC");
$resultado = mysqli_query($conectar, $sql_consulta);
while ($contenedor = mysqli_fetch_assoc($resultado)) {
...
<td><a href="#">MOTIVO2:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
<select name="motivo2" class="selectpicker" data-show-subtext="true" data-live-search="true">
<?php if (isset($motivo2)) { echo "<option value='-1' >...</option>";
} else { echo  "<option value='-1' selected >...</option>"; }
$sql_consulta = ("SELECT motivo FROM motivo_consulta GROUP BY motivo ORDER BY motivo DESC");
$resultado = mysqli_query($conectar, $sql_consulta);
while ($contenedor = mysqli_fetch_assoc($resultado)) {
